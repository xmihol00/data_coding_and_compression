...
private:
    ...
    } _indexPrefixLengthCodeCount[MAX_CODE_LENGTH] __attribute__((aligned(64)));

    struct IndexPrefixLength
    {
        uint16_t index;
        int16_t prefixLength;
    } _indexPrefixLengths[MAX_CODE_LENGTH] __attribute__((aligned(64)));
    ...
    } _depthsIndices[MAX_CODE_LENGTH] __attribute__((aligned(64)));
    ...
    uint16_t _prefixIndices[MAX_CODE_LENGTH];
    uint16_t _prefixShifts[1 << (sizeof(uint8_t) * 8)];
    uint16_t _suffixShifts[1 << (sizeof(uint8_t) * 8)];

    uint32v16_t _codePrefixesLargeVector[MAX_CODE_LENGTH / 16] __attribute__((aligned(64)));
    ...
    uint32v16_t _codeMasksLargeVector[MAX_CODE_LENGTH / 4] __attribute__((aligned(64)));
